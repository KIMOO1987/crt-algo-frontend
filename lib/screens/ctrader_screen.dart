...
class CTraderScreen extends StatefulWidget {
  const CTraderScreen({super.key});

  @override
  State<CTraderScreen> createState() => _CTraderScreenState();
}

class _CTraderScreenState extends State<CTraderScreen> {
  ...
  Future<void> _loadCTraderData() async {
    ...
    try {
      ...
      // 2. Load or insert cTrader configurations
      ...
      _showSnackBar('Error loading cTrader settings: $e', isError: true);
    } finally {
  ...
  Future<void> _toggleBotStatus(bool active) async {
    ...
    try {
      ...
      await _supabase.from('cbot_logs').insert({
        'bot_token': _botToken!,
        'message': 'CBot execution receiver toggled ${active ? 'STARTED' : 'STOPPED'} from Mobile client.',
        'log_type': 'INFO',
      });

      _showSnackBar(active ? 'Receiver Started Successfully!' : 'Receiver Stopped Successfully!');
      _loadLogs(_botToken!);
    } catch (e) {
      setState(() => _isBotActive = !active);
  ...
  Widget build(BuildContext context) {
    final webhookUrl = _botToken != null ? 'https://api.crtalgo.online/webhook/ctrader/$_botToken' : 'Loading...';

    return Scaffold(
      backgroundColor: const Color(0xFF07080D),
      drawer: const NavDrawer(activeRoute: 'ctrader'),
      appBar: AppBar(
        title: const Text(
