            ...
            return (
              ...
              >
                <div className={`w-3.5 h-3.5 rounded border flex items-center justify-center shrink-0 ${
                  checked ? 'border-cyan-500 bg-cyan-500 text-black' : 'border-zinc-300 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900'
                }`}>
                  {checked && (
...
export default function CTraderDashboard() {
    ...
    try {
      ...
      if (!sigData) {
        const { data: newData } = await supabase
          .from('bot_signals')
          .insert([{ user_id: uId, platform: 'cTrader' }])
          .select()
          .single();
        sigData = newData;
      }
      ...
      // Fetch cTrader configurations from ctrader_auth
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel('ctrader-realtime')
      .on(
        'postgres_changes',
        { event: '*', schema: 'public', table: 'trade_executions', filter: `user_id=eq.${userId}` },
        (payload) => {
          if (payload.new && (payload.new as any).exchange_name === 'ctrader') {
            fetchDashboardData();
          }
        }
      )
      .on(
        'postgres_changes',
        { event: '*', schema: 'public', table: 'ctrader_auth', filter: `user_id=eq.${userId}` },
        () => {
          fetchDashboardData();
        }
      );
  ...
  const handleControl = async (action: 'start' | 'stop') => {
    addLog(`Sending ${action.toUpperCase()} command to cTrader Cloud Bridge...`);
    try {
  ...
  const handleResetData = async () => {
    ...
    const confirmReset = window.confirm(
      "⚠️ DANGER ZONE: Are you sure you want to reset all your cTrader trade logs and executions?\n\nThis deletes your cTrader performance metrics permanently. This action cannot be undone."
    );
    if (!confirmReset) return;

    setIsResetting(true);
    addLog("⏳ Resetting cTrader logs and trade history...");
    try {
      await supabase.from('trade_executions').delete().eq('user_id', userId).eq('exchange_name', 'ctrader');
      await supabase.from('cbot_logs').delete().eq('bot_token', botToken);
      
      setRawExecutions([]);
      setBridgeLogs([]);
      addLog("✅ cTrader Reset Successful.", undefined, "SUCCESS");
      fetchDashboardData();
    } catch (err: any) {
  ...
  const exportToCSV = () => {
    ...
    link.setAttribute("download", `ctrader_trade_log_${new Date().toISOString().slice(0, 10)}.csv`);
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
  };

  const metalsList = CTRADER_SYMBOLS.filter(s => s.category === 'METALS').map(s => s.symbol);
  const indicesList = CTRADER_SYMBOLS.filter(s => s.category === 'INDICES').map(s => s.symbol);
  const forexList = CTRADER_SYMBOLS.filter(s => s.category === 'FOREX').map(s => s.symbol);
  const cryptoList = CTRADER_SYMBOLS.filter(s => s.category === 'CRYPTO').map(s => s.symbol);

  const filteredMetals = CTRADER_SYMBOLS.filter(s => 
    s.category === 'METALS' && s.symbol.toLowerCase().includes(searchQuery.toLowerCase())
  );
  const filteredIndices = CTRADER_SYMBOLS.filter(s => 
    s.category === 'INDICES' && s.symbol.toLowerCase().includes(searchQuery.toLowerCase())
  );
  const filteredForex = CTRADER_SYMBOLS.filter(s => 
    s.category === 'FOREX' && s.symbol.toLowerCase().includes(searchQuery.toLowerCase())
  );
  const filteredCrypto = CTRADER_SYMBOLS.filter(s => 
      ...
      <div className="w-full relative z-10 space-y-6 md:space-y-8 text-zinc-900 dark:text-white">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 mb-8 md:mb-12 border-b border-zinc-800 pb-8">
          <div>
            <h1 className="text-2xl md:text-4xl font-black tracking-tighter italic flex items-center gap-3 uppercase text-zinc-900 dark:text-white">
              ...
              cTrader<span className="text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-teal-500">Terminal</span>
            </h1>
          ...
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-3 px-5 py-2.5 bg-[var(--glass-bg)] border border-[var(--glass-border)] rounded-xl backdrop-blur-md shadow-xl">
              <div className={`w-2.5 h-2.5 rounded-full ${status === 'running' ? 'bg-cyan-400 shadow-[0_0_10px_rgba(6,182,212,0.8)]' : 'bg-red-400 shadow-[0_0_10px_rgba(248,113,113,0.8)]'} animate-pulse`} />
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
          ...
          <div className="glass-panel p-5">
            ...
            <p className="text-2xl font-black mt-2 font-mono text-cyan-400">✓ {metrics.fullTps}</p>
          </div>
          <div className="glass-panel p-5">
        ...
        <div className="glass-panel p-8 space-y-6">
          <h3 className="text-xs font-black text-cyan-500 uppercase tracking-widest flex items-center gap-2">
            <BarChart3 size={16} /> cTrader Performance Board
          </h3>
        ...
        <div className="glass-panel p-8 space-y-6">
          <div className="flex justify-between items-center flex-wrap gap-4 border-b border-zinc-850 pb-4">
            <div>
              <h2 className="text-[12px] font-black uppercase tracking-[0.2em] text-cyan-500">
            ...
            <input
              ...
              className="bg-white/50 dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-lg px-4 py-2 text-xs outline-none w-48 focus:border-cyan-500 transition-all font-mono text-zinc-900 dark:text-white"
            />
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              ...
              <div className="flex justify-between items-center border-b border-zinc-200 dark:border-zinc-900 pb-3">
                <div>
                  <h3 className="text-xs font-black text-zinc-900 dark:text-white uppercase tracking-wider flex items-center gap-2">
                    <Flame size={14} className="text-cyan-400" /> Metals
                  </h3>
                ...
                {filteredMetals.map(sym => {
                  ...
                  return (
                    <div key={sym.symbol} className="flex items-center justify-between p-2.5 bg-zinc-100/10 dark:bg-zinc-950/20 border border-zinc-200 dark:border-zinc-850/60 rounded-xl hover:border-zinc-350 dark:hover:border-zinc-800 transition-all select-none">
                      ...
                      >
                        <div className={`w-3.5 h-3.5 rounded border flex items-center justify-center shrink-0 ${
                          checked ? 'border-cyan-500 bg-cyan-500 text-black' : 'border-zinc-300 dark:border-zinc-850 bg-zinc-50 dark:bg-zinc-900'
                        }`}>
                          {checked && (
                            <svg className="w-2.5 h-2.5 fill-current stroke-2" viewBox="0 0 24 24">
                              <path fill="none" stroke="currentColor" strokeWidth="4" d="M5 13l4 4L19 7" />
            ...
            <div className="bg-zinc-100/30 dark:bg-zinc-950/40 border border-zinc-200 dark:border-zinc-900 p-6 rounded-2xl space-y-4">
              <div className="flex justify-between items-center border-b border-zinc-200 dark:border-zinc-900 pb-3">
                <div>
                  <h3 className="text-xs font-black text-zinc-900 dark:text-white uppercase tracking-wider flex items-center gap-2">
                    <BarChart3 size={14} className="text-cyan-400" /> Indices
                  </h3>
                  <span className="text-[8px] font-mono text-zinc-550 dark:text-zinc-500 uppercase">
                    ...
                    return (
                      <button
                        ...
                        className={`px-1.5 py-0.5 rounded text-[8px] font-bold font-mono tracking-tighter uppercase transition-all border shrink-0 cursor-pointer ${
                          bulkChecked 
                            ? 'border-cyan-500 bg-cyan-500/10 text-cyan-400' 
                            : 'border-zinc-200 dark:border-zinc-800 text-zinc-500 hover:text-zinc-900 dark:hover:text-zinc-355 font-bold'
                        }`}
                      >
                        {tf.split("/")[0]}
                      </button>
            ...
            <div className="bg-zinc-100/30 dark:bg-zinc-950/40 border border-zinc-200 dark:border-zinc-900 p-6 rounded-2xl space-y-4">
              <div className="flex justify-between items-center border-b border-zinc-200 dark:border-zinc-900 pb-3">
                <div>
                  <h3 className="text-xs font-black text-zinc-900 dark:text-white uppercase tracking-wider flex items-center gap-2">
                    <Compass size={14} className="text-cyan-400" /> Forex
                  </h3>
                  <span className="text-[8px] font-mono text-zinc-550 dark:text-zinc-500 uppercase">
            ...
            <div className="bg-zinc-100/30 dark:bg-zinc-950/40 border border-zinc-200 dark:border-zinc-900 p-6 rounded-2xl space-y-4">
              <div className="flex justify-between items-center border-b border-zinc-200 dark:border-zinc-900 pb-3">
                <div>
                  <h3 className="text-xs font-black text-zinc-900 dark:text-white uppercase tracking-wider flex items-center gap-2">
                    <Award size={14} className="text-cyan-400" /> Crypto (Major Only)
                  </h3>
                  <span className="text-[8px] font-mono text-zinc-550 dark:text-zinc-500 uppercase">
                ...
                {filteredCrypto.map(sym => {
                  ...
                  return (
                    <div key={sym.symbol} className="flex items-center justify-between p-2.5 bg-zinc-100/10 dark:bg-zinc-950/20 border border-zinc-200 dark:border-zinc-850/60 rounded-xl hover:border-zinc-350 dark:hover:border-zinc-800 transition-all select-none">
                      ...
                      >
                        <div className={`w-3.5 h-3.5 rounded border flex items-center justify-center shrink-0 ${
                          checked ? 'border-cyan-500 bg-cyan-500 text-black' : 'border-zinc-305 dark:border-zinc-855 bg-zinc-50 dark:bg-zinc-900'
                        }`}>
                          {checked && (
                            <svg className="w-2.5 h-2.5 fill-current stroke-2" viewBox="0 0 24 24">
                              <path fill="none" stroke="currentColor" strokeWidth="4" d="M5 13l4 4L19 7" />
        ...
        <div className="glass-panel p-6 flex flex-col sm:flex-row justify-between items-center gap-4">
          <p className="text-[11px] text-zinc-400 tracking-wide font-bold uppercase text-center sm:text-left leading-relaxed">
            💡 Allowed symbols configuration dynamically overrides signal dispatches for cTrader bots in real-time.
          </p>
          <button
            onClick={saveSettings}
            disabled={isSaving}
            className="px-8 py-3.5 bg-gradient-to-r from-cyan-600 to-teal-600 hover:from-cyan-500 hover:to-teal-500 border border-cyan-500/20 text-white text-[11px] font-black uppercase tracking-widest rounded-xl transition-all shadow-xl active:scale-95 flex items-center gap-2 cursor-pointer disabled:opacity-50 shrink-0 w-full sm:w-auto justify-center"
          >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          <div className="glass-panel p-6 md:p-8 space-y-6">
            <h2 className="text-[11px] font-black uppercase tracking-widest flex items-center gap-3">
              <Power size={16} className="text-cyan-400" /> Engine Controls
            </h2>
            <div className="space-y-4">
              <button
                ...
                className={`w-full py-4 rounded-xl text-[11px] font-black uppercase tracking-widest transition-all duration-300 flex items-center justify-center gap-3 active:scale-95 ${status === 'running'
                  ...
                  : 'bg-gradient-to-r from-cyan-600 to-teal-600 hover:from-cyan-500 hover:to-teal-500 text-white shadow-[0_0_30px_rgba(6,182,212,0.3)] hover:shadow-[0_0_40px_rgba(6,182,212,0.5)] border border-cyan-500/30'
                  }`}
                disabled={status === 'running'}
              >
                <Power size={16} className={status === 'running' ? 'text-zinc-600' : 'text-cyan-200'} /> CONNECT ENGINE
              </button>
              <button
          ...
          <div className="glass-panel p-6 md:p-8 space-y-6">
            ...
            <div className="space-y-4">
              <p className="text-zinc-400 text-xs leading-relaxed">
                Clear all historical trade logs, execution files, and cTrader performance metrics. Your Allowed Symbols checklists and bot credentials will not be affected.
              </p>
              <button
                    ...
                    {rawExecutions.length === 0 ? (
                      <tr>
                        <td colSpan={9} className="py-6 text-center text-zinc-500 font-bold uppercase tracking-wider">
                          No cTrader executions recorded yet.
                        </td>
                      </tr>
                    ) : (
                        ...
                        <tr key={e.id} className="hover:bg-zinc-100/50 dark:hover:bg-zinc-900/10 transition-colors">
                          ...
                          <td className="py-3 text-right whitespace-nowrap">
                            <span className={`px-2 py-0.5 rounded text-[8px] font-black uppercase tracking-wider ${
                              ...
                              ['TP2_HIT', 'WIN'].includes(e.status) ? 'bg-emerald-500/20 text-emerald-655 dark:text-emerald-400 border border-emerald-500/30' :
          ...
          <div className="max-w-4xl mx-auto space-y-12">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-16 gap-y-12">
              {[
                ...
                { title: 'Engine Start', desc: 'Click the "CONNECT ENGINE" button at the top of this page, then click "Start" on your cBot instance in cTrader.' }
              ].map((step, i) => (
                <div key={i} className="flex gap-6">
                  <div className="shrink-0 w-10 h-10 rounded-xl bg-cyan-500/10 border border-cyan-500/20 flex items-center justify-center text-cyan-600 dark:text-cyan-400 font-black italic">
                    0{i + 1}
                  </div>
