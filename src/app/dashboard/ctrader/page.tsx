...
export default function CTraderDashboard() {
  ...
  const majorsList = CTRADER_SYMBOLS.filter(s => s.type === 'major').map(s => s.symbol);
  const altsList = CTRADER_SYMBOLS.filter(s => s.type === 'alt').map(s => s.symbol);

  const filteredMajors = CTRADER_SYMBOLS.filter(s => 
    s.type === 'major' && s.symbol.toLowerCase().includes(majorSearchQuery.toLowerCase())
  );
  
  const filteredAlts = CTRADER_SYMBOLS.filter(s => 
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="glass-panel p-8 space-y-6">
            <div className="flex justify-between items-center flex-wrap gap-4 border-b border-zinc-800 pb-4">
              <div>
                <h3 className="text-xs font-black text-cyan-500 uppercase tracking-widest flex items-center gap-2">
                  <Flame size={16} className="text-cyan-400" /> Major Symbols
                </h3>
              ...
              <input
                ...
                className="bg-zinc-950 border border-zinc-800 rounded-lg px-3 py-1.5 text-[10px] outline-none w-32 focus:border-cyan-500 transition-all font-mono"
              />
            </div>
          ...
          <div className="glass-panel p-8 space-y-6">
            <div className="flex justify-between items-center flex-wrap gap-4 border-b border-zinc-800 pb-4">
              <div>
                <h3 className="text-xs font-black text-cyan-500 uppercase tracking-widest flex items-center gap-2">
                  <Compass size={16} className="text-cyan-400" /> Alt Symbols
                </h3>
                <span className="text-[9px] font-mono text-zinc-500 uppercase tracking-wider">
